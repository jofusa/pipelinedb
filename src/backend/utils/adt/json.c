...
jsonaggstatesend(PG_FUNCTION_ARGS)
{
	...
	pos = VARDATA(result);
	memcpy(pos, state, sizeof(JsonAggState));
	pos += sizeof(JsonAggState);
	memcpy(pos, state->str->data, state->str->len);
...
jsonaggstaterecv(PG_FUNCTION_ARGS)
{
	...
	pos = (char *) VARDATA_ANY(bytes);
	nbytes = VARSIZE_ANY(bytes);

	result = palloc(sizeof(JsonAggState));
	memcpy(result, pos, sizeof(JsonAggState));
	pos += sizeof(JsonAggState);

	result->str = makeStringInfo();
	appendBinaryStringInfo(result->str, pos, nbytes - sizeof(JsonAggState));
